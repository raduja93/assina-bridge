...
// Para subaccount, priorize APP_ID (foi o que funcionou no create). Cai para API_TOKEN se faltar.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!WOOVI_PRIMARY_CRED) {
    return res.status(500).json({ ok:false, error:"missing_woovi_credentials", hint:"Defina WOOVI_APP_ID ou WOOVI_API_TOKEN" });
  }
  ...
  try {
    ...
    if (!id || typeof id !== "string" || !id.trim()) {
      return res.status(400).json({ ok:false, error:"missing_id", hint:"Envie ?id=... ou body { id: \"...\" }" });
    }
    ...
    const r = await axios.delete(url, {
      headers: {
        ...
        "X-Api-Key": WOOVI_PRIMARY_CRED, // sem prejuízo se não for usado nesse endpoint
        "Content-Type": "application/json",
      },
      // validateStatus: () => true, // habilite se quiser tratar manualmente códigos 4xx/5xx
    });

    // Se a Woovi retornar 2xx, consideramos apagado
